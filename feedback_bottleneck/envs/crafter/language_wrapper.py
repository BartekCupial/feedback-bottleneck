...
from feedback_bottleneck.envs.crafter.actions import CRAFTER_ACTIONS
...
def describe_frame(
    ...
    try:
        ...
        print(f"Warning: Error in describe_frame: {e}")
        # Try to return inventory if possible, otherwise fallback message
        try:
            inventory = describe_inventory(info)
        except:
            inventory = "Your status and inventory are unavailable."
        return "Error, you are out of the map, last action replaced with Noop.", inventory
...
class CrafterLanguageWrapper(gym.Wrapper):
    ...
    def __init__(
        ...
        self.language_action_space = Strings(CRAFTER_ACTIONS)
    ...
    def _is_valid_move(self, action):
        """Check if movement action would keep player within map boundaries."""
        if action not in [1, 2, 3, 4]:  # Move West, East, North, South
            return True  # Non-movement actions are always valid

        try:
            player_pos = self.env.unwrapped._player.pos
            world_size = self.env.unwrapped._size
            ...
            if action == 1:  # Move West
                ...
                new_pos[0] += 1
            elif action == 3:  # Move North
                new_pos[1] -= 1
            elif action == 4:  # Move South
                new_pos[1] += 1

            # Check if new position is within bounds
            return 0 <= new_pos[0] < world_size[0] and 0 <= new_pos[1] < world_size[1]
        except:
            # If we can't check bounds, allow the action (fallback)
            return True
    ...
    def _step_impl(self, action):
        ...
        aug_info = info.copy()
        aug_info["sleeping"] = self.env.unwrapped._player.sleeping
        aug_info["player_facing"] = self.env.unwrapped._player.facing
        aug_info["dead"] = self.env.unwrapped._player.health <= 0
        aug_info["unlocked"] = {
            name
            for name, count in self.env.unwrapped._player.achievements.items()
            if count > 0 and name not in self.env.unwrapped._unlocked
        }
        aug_info["view"] = self.env.unwrapped._view
    ...
    def step(self, action):
        mapped_action = self.language_action_space.map(action)

        # Check if movement action would go out of bounds
        if not self._is_valid_move(mapped_action):
            # Replace with Noop action to prevent going out of bounds
            mapped_action = 0

        obs, reward, done, info = self._step_impl(mapped_action)
        self.score_tracker = self.update_progress(info)
        obs = self.process_obs(obs, info)
        return obs, reward, done, info

    def process_obs(self, obs, info):
        img = Image.fromarray(self.env.unwrapped.render()).convert("RGB")
        ...
        new_obs = {}
        # dont include images in crafter as they are too redundant and heavy
        # new_obs["data"] = obs
        new_obs["long_term_context"] = long_term_context
        new_obs["short_term_context"] = short_term_context
