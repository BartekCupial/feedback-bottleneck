...
class VerifierEngine:
    ...
    def __init__(self, mcp_clients: Union[MCPClient, Dict[str, MCPClient]], llm_client: Optional[Any] = None):
        """
        ...
        if isinstance(mcp_clients, dict):
            ...
            self.mcp_client = list(mcp_clients.values())[0] if mcp_clients else None  # Backward compat
        else:
    ...
    async def execute_verifier(
        ...
        if verifier.verifier_type == "database_state":
            ...
            return await self._execute_tool_execution_verifier(verifier.validation_config, model_response)
        else:
            return {
                "passed": False,
    ...
    async def _execute_database_state_verifier(
        ...
        result = await self._execute_sql_query(sql_query, database_id, context, gym_name)
    ...
    async def _execute_response_check_verifier(
        ...
        # Execute SQL query
        sql_result = await self._execute_sql_query(sql_query, database_id, context, gym_name)
        ...
        try:
            ...
            if hasattr(e, "response") and e.response is not None:
                try:
                    ...
                    logger.error(f"HTTP error calling sql-runner API: {status_code} - {error_details}")
