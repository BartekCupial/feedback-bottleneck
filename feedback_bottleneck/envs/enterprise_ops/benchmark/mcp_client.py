...
def create_database_from_file(gym_url: str, sql_file_path: str) -> Optional[str]:
    ...
    try:
        ...
        logger.info(f"🔨 Creating database '{db_name}' from file on {gym_url}...")
        payload = {
        ...
        logger.error(f"❌ Error creating database from file on {gym_url}: {e}")
        raise
    ...
    try:
        ...
        with httpx.Client(timeout=30) as client:
            ...
            if response.status_code == 404:
                logger.warning("⚠️  Server does not support database deletion API")
                return False
            elif response.status_code == 405:
                logger.warning("⚠️  Database deletion not allowed on this server")
                return False
        ...
        if e.response.status_code in [404, 405]:
            logger.warning(f"⚠️  Server does not support database deletion (HTTP {e.response.status_code})")
        else:
    ...
    async def _send_request(
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=timeout) as client:
                logger.info(f"Sending MCP request: {method} to {self.base_url}{self.mcp_endpoint}")
                logger.debug(f"Headers: {headers}")
                logger.debug(f"Payload: {json.dumps(payload, indent=2)}")

                response = await client.post(f"{self.base_url}{self.mcp_endpoint}", json=payload, headers=headers)
                ...
                if response.status_code == 200:
                    ...
                    error_msg = f"MCP request failed: {response.status_code} - {response.text}"
