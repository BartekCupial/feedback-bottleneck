...
from feedback_bottleneck.envs.bbeh.instruction_prompt import get_refinement_prompt_spec
...
def _build_bbeh_observation(step_data: Dict[str, Any]) -> Dict[str, Any]:
    return {
        "obs": {
...
class NaiveBBehFormatter(NaiveRefinementFormatter):
    def __init__(self, args: Args):
        super().__init__(
            args,
            prompt_spec=get_refinement_prompt_spec(args.task, prompt_args=args.refinement_prompt),
            step_observation_builder=_build_bbeh_observation,
        )


class HierarchicalBBehFormatter(HierarchicalRefinementFormatter):
    def __init__(self, args: Args):
        super().__init__(
            args,
            prompt_spec=get_refinement_prompt_spec(args.task, prompt_args=args.refinement_prompt),
...
class NaiveBBehAgent(NaiveRefinementAgent):
    def __init__(self, llm_actor, prompt_formatter, args: Args):
        super().__init__(llm_actor, prompt_formatter, args, answer_tag="answer")


class HierarchicalBBehAgent(HierarchicalRefinementAgent):
    def __init__(self, llm_actor, prompt_formatter, args: Args):
        super().__init__(llm_actor, prompt_formatter, args, answer_tag="answer")


class HierarchicalSeparateBBehAgent(HierarchicalSeparateRefinementAgent):
