...
from feedback_bottleneck.envs.countdown.instruction_prompt import get_refinement_prompt_spec
...
def _build_countdown_observation(step_data: Dict[str, Any]) -> Dict[str, Any]:
    return {
        "obs": {
            "problem": step_data["obs_problem"],
            "problem_id": step_data["obs_problem_id"],
            "source": step_data["obs_source"],
            "target": step_data["obs_target"],
            "numbers": step_data["obs_numbers"],
...
class NaiveCountdownFormatter(NaiveRefinementFormatter):
    def __init__(self, args: Args):
        super().__init__(
            args,
            prompt_spec=get_refinement_prompt_spec(args.task, prompt_args=args.refinement_prompt),
            step_observation_builder=_build_countdown_observation,
        )


class HierarchicalCountdownFormatter(HierarchicalRefinementFormatter):
    def __init__(self, args: Args):
        super().__init__(
            args,
            prompt_spec=get_refinement_prompt_spec(args.task, prompt_args=args.refinement_prompt),
...
class NaiveCountdownAgent(NaiveRefinementAgent):
    def __init__(self, llm_actor, prompt_formatter, args: Args):
        super().__init__(llm_actor, prompt_formatter, args, answer_tag="answer")


class HierarchicalCountdownAgent(HierarchicalRefinementAgent):
    def __init__(self, llm_actor, prompt_formatter, args: Args):
        super().__init__(llm_actor, prompt_formatter, args, answer_tag="answer")


class HierarchicalSeparateCountdownAgent(HierarchicalSeparateRefinementAgent):
